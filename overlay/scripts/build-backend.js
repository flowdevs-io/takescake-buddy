...
const outputFile = path.join(outputDir, 'mtga-tracker-backend.exe');
const target = process.env.BUN_BACKEND_TARGET || 'bun-windows-x64';

fs.mkdirSync(outputDir, { recursive: true });
fs.rmSync(outputFile, { force: true });

console.log(`[backend:build] Building ${outputFile} with target ${target}`);

const result = spawnSync(
  'bun',
  [
    'build',
    path.join(repoRoot, 'scraper', 'ui.ts'),
    '--compile',
    `--target=${target}`,
    '--minify',
    '--sourcemap',
